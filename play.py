...
def run(playwright: Playwright,email,direk,nmFile)-> None:
  ...
  if page.query_selector('//*[@class="confirm-item"]'):
    page.locator('//*[@class="confirm-item"]').click()
  ...
  while True:
    ...
    try:
      token = re.findall("ya29.*?quot",newPage.content())[0].replace("\&quot",'')
      break
    except:pass
  newPage.wait_for_timeout(3000)
  context.storage_state(path=f"MyCookies/{direk}/{nmFile}.json")
  context.close()
  browser.close()
  return token
